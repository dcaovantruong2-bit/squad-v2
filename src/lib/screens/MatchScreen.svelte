<script>
  ...
  $: contributorIdxs = ($game.field || [])
    ...
    .filter(i => i >= 0);
  $: resting = ($game.field || []).filter(e => !phasePositions.has(e.position));
  ...
  function flyIntoShape() {
    const pitch = pitchEl;
    if (!pitch) return;
    const dots = pitch.querySelectorAll('.player-dot');
    if (!dots.length) return;
    const reduced = typeof window !== 'undefined' && window.matchMedia?.('(prefers-reduced-motion: reduce)')?.matches;
    sfx.kickoff(); // referee whistle — the phase is under way
    dots.forEach(dot => {
  ...
  // Re-run the fly whenever the live phase changes (entry + after each whistle),
  // once the DOM has the new dots in place.
  $: current && tick().then(() => flyIntoShape());
</script>
      ...
      <div class="match-pitch" bind:this={pitchEl}>
        ...
        {#each contributorSlots as cs (cs.i)}
          ...
          <button
            ...
            class="player-dot"
            class:low={cur <= 1}
            disabled={!current || !swappableSlots.has(cs.i)}
            on:click={() => overlaySlot = cs.i}
            aria-label={`${cs.player.name} (${cs.pos}) — tap to rearrange`}
            data-fx={cs.from?.x ?? 50} data-fy={cs.from?.y ?? 50}
            data-tx={cs.x} data-ty={cs.y} data-role={cs.pos}
            style={`left:clamp(52px, ${cs.x}%, calc(100% - 52px));top:clamp(30px, ${cs.y}%, calc(100% - 30px))`}
          >
            <span>{cs.pos}</span>
